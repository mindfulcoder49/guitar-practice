...
import { Play, Square, RotateCcw, ArrowLeft, BookmarkPlus } from 'lucide-react'
import { toast } from 'sonner'
import { notFound } from 'next/navigation'

function PlayContent({ songId }: { songId: string }) {
  const songOrPattern = getSongOrPattern(songId)
  if (!songOrPattern) notFound()

  const [stream, setStream] = useState<MediaStream | null>(null)
  const [running, setRunning] = useState(false)
  const [bpm, setBpm] = useState(songOrPattern.bpm)
    ...
    try {
      ...
      if (accuracy >= 0.6) {
        await fetch('/api/songs', {
          ...
          body: JSON.stringify({
            type: 'learnSong',
            data: { songId, accuracy },
          }),
        })
        toast.success(`Great job! ${Math.round(accuracy * 100)}% accuracy — marked as learned!`)
      } else {
  ...
  async function handleSaveToCatalog() {
    try {
      await fetch('/api/catalog', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: songOrPattern.title,
          type: songOrPattern.type,
          data: JSON.stringify(songOrPattern),
        }),
      })
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-4xl">
      <Link href={`/songs/${songId}`} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground mb-4">
        <ArrowLeft className="w-4 h-4" />
        Back
      </Link>

      <div className="mb-4">
        <h1 className="text-xl sm:text-2xl font-bold">{songOrPattern.title}</h1>
        {'artist' in songOrPattern && songOrPattern.artist && (
          <p className="text-muted-foreground text-sm">{songOrPattern.artist}</p>
        )}
        {'chordName' in songOrPattern && (
          <p className="text-muted-foreground text-sm">Hold: <strong>{songOrPattern.chordName}</strong></p>
        )}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4 order-2 lg:order-1">
          ...
          <NoteHighway
            notes={songOrPattern.type === 'melody' ? songOrPattern.notes : undefined}
            pattern={songOrPattern.type === 'fingerpick' ? songOrPattern.sequence : undefined}
            patternLoops={'loops' in songOrPattern ? songOrPattern.loops : 4}
            bpm={bpm}
            running={running}
...
export default function SongPlayPage({ params }: { params: Promise<{ songId: string }> }) {
  const { songId } = use(params)
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <Suspense fallback={<div className="container mx-auto px-4 py-8">Loading...</div>}>
        <PlayContent songId={songId} />
      </Suspense>
    </div>
  )
}
